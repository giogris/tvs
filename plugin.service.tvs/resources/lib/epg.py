...
import db
import xmltvfunc
from util import * #purge_string, DEFAULT_ENCODING, get_country, download, decodestr
				...
				try:
					...
					for program in programs:
						...
						if program.hasAttribute('channel'):
							channel = decodestr(program.getAttribute('channel'))
							if channel <> None:
								title = None
								subtitle = ''
								category = ''
										...
										if node.nodeName == 'title' and node.hasChildNodes:
											...
											for nc in node.childNodes:
												if nc.nodeType == program.TEXT_NODE:
													title += decodestr(nc.nodeValue)
												plugin.log.info('epg->['+channel+'] '+program.getAttribute('start')+' '+title)
										elif node.nodeName == 'sub-title' and node.hasChildNodes:
												subtitle = ''
												for nc in node.childNodes:
													if nc.nodeType == program.TEXT_NODE:
														subtitle += decodestr(nc.nodeValue)
										elif node.nodeName == 'desc' and node.hasChildNodes:
												description = ''
												for nc in node.childNodes:
													if nc.nodeType == program.TEXT_NODE:
														description += decodestr(nc.nodeValue)
										elif node.nodeName == 'category' and node.hasChildNodes:
											category = ''
											for nc in node.childNodes:
												if nc.nodeType == program.TEXT_NODE:
													category += decodestr(nc.nodeValue)
				...
				try:
					...
					for channel in channels:
						...
						if channel.hasAttribute('id'):
							item['id'] = decodestr(channel.getAttribute('id'))
						names = channel.getElementsByTagName('display-name')
						if len(names) > 0:
							item['name'] = purge_string(decodestr(names[0].childNodes[0].data).upper())
