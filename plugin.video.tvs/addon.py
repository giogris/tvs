...
import xbmcgui
import db
from util import get_country, DEFAULT_ENCODING, purge_string, decodestr, try_parse_int
from resources.lib import config
from resources.lib import xbmcutil
from resources.lib import vlctelnet
from resources.lib.dvbtuner import dvb_tune_adapter, tvdelete_channel
from resources.lib.xbmcutil import TVSError
from xbmcswift2 import Plugin, SortMethod, ListItem, actions
...
def show_root():    
    ...
    if len(config.tvconfig.adapters) == 0:
        ...
        for i in range(len(config.tvconfig.adapters)):
            ...
            item = {
                    'label': config.tvconfig.adapters[i][0],
                    'path': plugin.url_for(str('show_adapter'),adapter=str(i)),
                    'icon': xbmcutil.get_imagepath('ico_adapter.png'),
                    'is_playable': False
                    }
            items.append(item)            
...
def show_adapter(adapter):
    items = []
    item = {
            'label': xbmcutil.translate('channels'),
            'path': plugin.url_for(str('show_channels'),adapter=adapter),
            'icon': xbmcutil.get_imagepath('ico_channels.png'),
            'is_playable': False
            }
    items.append(item)
    item = {
...
def play_channel(adapter, channelid):
    ...
    def tune_and_play(adapter,channelid):
        ...
        plugin.log.debug('selected for play %s://%s:%s/%d' % (streaming_protocol,streaming_ip,streaming_port,ichannel))
        if channelplay <> '':
        ...
        if not player.channel_isplaying(ichannel):
            ...
            time.sleep(1)
        return (str('%s://%s:%s/%d' % (streaming_protocol,streaming_ip,streaming_port,ichannel)))
    # ...and now play the tv stream...
    iadapter = int(adapter)
    ichannel = int(channelid)
    plugin.log.debug('TVItem call...')
    item = TVItem(adapter,db.read_channel_by_id(iadapter,ichannel))
    plugin.log.debug('TVPlayer call...')
    xplayer = TVPlayer(int(adapter))
    plugin.log.debug('tune and play call...')
    xplayer.play(tune_and_play(adapter,channelid), item.as_xbmc_listitem()) 
    return plugin.end_of_directory(succeeded=False, update_listing=True, cache_to_disc=False)
    #return plugin.set_resolved_url(tune_and_play(adapter,channelid))
...
def create_context_menu(name,adapter,id):
    context_menu = [
                    (xbmcutil.translate('mnu_epg_guide') % (name), 'RunPlugin(%s)' % (plugin.url_for('epg_channel', adapter=adapter, channelid=str(id)))),
                    (xbmcutil.translate('mnu_record_start') % (name), 'RunPlugin(%s)' % (plugin.url_for('rec_channel', adapter=adapter, channelid=str(id)))),
                    (xbmcutil.translate('mnu_delete') % (name), 'RunPlugin(%s)' % (plugin.url_for('del_channel', adapter=adapter, channelid=str(id))))
                ]
    return context_menu
...
class TVPlayer(xbmc.Player):
    def __init__ (self, adapter=str('0')):
        self.dialog = None
        self.adapter = str(adapter)
        xbmc.Player.__init__(self)        
        plugin.log.debug('Initializing TVPlayer...')
    
    def play(self, url, listitem):
        plugin.log.debug('Now playing... %s' % url)
    ...
    def onPlayBackEnded(self):
        # refresh channels list to display new epg data and more...
        plugin.log.debug('onPlayBackEnded detected, refresh channel list...')
        xbmc.executebuiltin('XBMC.Container.Update(plugin://'+PLUGIN_ID+'/'+self.adapter+'/channels')   
    
    def onPlayBackStopped(self):
        # refresh channels list to display new epg data and more...
        plugin.log.debug('onPlayBackStopped detected, refresh channel list...')
        xbmc.executebuiltin('XBMC.Container.Update(plugin://'+PLUGIN_ID+'/'+self.adapter+'/channels')   

def TVItem(adapter,channel):
    item = ListItem(label=channel.name,
                    label2=channel.nowstarttimeandtitle(),
    ...
    item.set_info('video',
                    {
                        ...
                        'tvshowtitle': channel.nowstarttimeandtitle(),
                        ...
                        'album': channel.nowstarttimeandtitle(),  # I used album 'cause I haven't found another filed to display label2
    ...
    item.add_stream_info('video',{'duration': try_parse_int(channel.v_nowduration)*60})
    return item    
